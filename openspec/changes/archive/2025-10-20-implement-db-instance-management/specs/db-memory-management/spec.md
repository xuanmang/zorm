...
定义 ZORM DB 实例的内存管理策略和模式，包括 Allocator 使用、资源清理、Arena 优化和泄漏检测。符合 Zig 语言内存管理最佳实践和功能规格说明书 2.1.2 节要求。
...
### 依赖于
- `std.mem.Allocator` - 内存分配器接口
- `std.heap.ArenaAllocator` - Arena 分配器
- `std.testing.allocator` - 测试用分配器
...
- [ ] 所有内存分配都显式传递 Allocator
- [ ] 所有 init 方法使用 errdefer 清理
