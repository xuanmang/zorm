...
- DeleteQuery 核心实现位于 `src/query/query.zig:1721-2107`
- has_where 字段用于强制 WHERE 检查
- whereIn/whereNotIn 实现复用 UPDATE Builder 的模式
- RETURNING 仅在支持的方言（PostgreSQL）中启用（comptime 检查）

## Test Coverage

必须覆盖以下场景：
- ✅ 基本 DELETE with WHERE
- ✅ 多个 WHERE 条件（AND/OR）
- ✅ RETURNING 子句
- ✅ WHERE 强制检查（MissingWhereClause）
- ⏳ whereIn 基本功能
- ⏳ whereNotIn 基本功能
- ⏳ whereIn 与 where 组合
- ⏳ whereIn 空列表错误
...
- select-query-api: SELECT Query Builder
- insert-query-api: INSERT Query Builder
- update-query-api: UPDATE Query Builder（whereIn 实现参考）
- bulk-update-api: Bulk UPDATE Support
