...
### Requirement: scan() 方法使用 collectArgs()

SELECT 查询构建器的 `scan()` 方法 MUST 使用 `collectArgs()` 收集参数，替代现有的参数管理逻辑。

#### Scenario: scan() 正确传递参数
...
for (users) |user| {
    try std.testing.expect(user.age > 18);
    try std.testing.expectEqualStrings("active", user.status);
}
```

---

### Requirement: scanOne() 方法使用 collectArgs()

SELECT 查询构建器的 `scanOne()` 方法 MUST 使用 `collectArgs()` 收集参数。

#### Scenario: scanOne() 正确传递参数
...
try std.testing.expectEqual(@as(i64, 123), user.id);
```

---

### Requirement: count() 方法使用 collectArgs()

SELECT 查询构建器的 `count()` 方法 MUST 使用 `collectArgs()` 收集参数。

#### Scenario: count() 正确传递参数
