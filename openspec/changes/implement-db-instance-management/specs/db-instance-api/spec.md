...
定义 ZORM DB 实例的核心 API 接口，包括实例创建、销毁、克隆、查询执行和查询构建器工厂方法。严格符合功能规格说明书 2.1.1 节的要求。
...
### 依赖于
- `driver.connection.Conn` - 连接接口
- `driver.connection.Result` - 查询结果接口
- `driver.connection.Tx` - 事务接口
- `hooks.QueryHook` - 查询钩子接口
- `types.QueryArg` - 查询参数类型
- `dialect.Dialect` - 方言枚举

### 被依赖于
- 查询构建器（SelectQuery, InsertQuery 等）
- 事务管理（Transaction, TxManager）
- Schema 管理（CreateTable, Migration 等）
